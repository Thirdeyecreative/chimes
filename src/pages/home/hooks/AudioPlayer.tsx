...
const AudioPlayer = () => {
  // Use isPlaying from Context (synced by PersistentAudio)
  const { audioRef, isPlaying } = useAuthContext();
  const [hasInteractedLocal, setHasInteractedLocal] = useState(false);

  // Toggle function acts on the ref (controlled by PersistentAudio)
